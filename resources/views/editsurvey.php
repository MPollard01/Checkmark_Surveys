...
<form id="survey-form" class="form-control shadow-sm border-0 row g-3 mx-auto pb-4" style="width: 60%" action="create_survey.php" method="post">
	<div class="col-12 mb-3">
		<input class="form-control form-control-lg input-border" type="text" name="title" placeholder="Survey Title" value="<?php echo $survey['title'] ?>" required> 
	</div>
    <?php
        foreach($survey['questions'] as $value)
        {
            ...
            echo'<div class="input-wrap row g-3">
                <div class="col-md-7">
                    <input type="text" name="question[]" class="form-control input-border" placeholder="Question" value="'.$value->text.'">
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-1">
                    <button type="button" class="btn btn-sm btn-outline-secondary remove-question" data-bs-toggle="tooltip" data-bs-placement="right" title="Remove question">
            ...
            {
                foreach($value->options as $k => $opt)
                {
                    echo '<div class="row g-3 question-type">
                        <div class="qtype col-md-1 d-flex justify-content-center align-items-center">
                            <input class="form-check-input" type="'.(($value->type === 'radio') ? 'radio' : 'checkbox').'" disabled>
                        </div>
                        <div class="col-md-6">
                            <input class="rd form-control input-border" type="text" name="options[]" placeholder="Option" value="'.$opt.'" required>
                        </div>
                        <div class="col-md-2">
                            '.(($k === 0) ? '<button type="button" class="add btn btn-sm btn-outline-secondary" data-bs-toggle="tooltip" data-bs-placement="right" title="Add option"><i class="fa fa-plus"></i></button>'
                            : '<button type="button" class="remove btn btn-sm btn-outline-secondary" data-bs-toggle="tooltip" data-bs-placement="right" title="Remove option"><i class="fa fa-times"></i></button>').'
                        </div>
                    </div>';
                }
                echo '</div>';
...
<script src="<?php echo $_ENV['APP_URL'] ?>/resources/assets/js/survey.js"></script>
